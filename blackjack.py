...
def _deal_card(frame):
    # pop the card off the top of the deck
...
def score_hand(hand):
    ...
    # only 1 ace can have the value of 11, and this will be reduced to 1 if hand busts
    score = 0
    ace = False
    for next_card in hand:
        ...
        if card_value == 1 and not ace:
            ...
            card_value = 11
        score += card_value
        # if we would bust, check if there is an ace and subtract 10 if that is the case
        if score > 21 and ace:
            score -= 10
            ace = False
    return score
...
# Set up screen and frames for dealer player
mainWindow.title("Black Jack")
mainWindow.geometry("640x480")
mainWindow.configure(background='green')

result_text = tkinter.StringVar()
result = tkinter.Label(mainWindow, textvariable=result_text, background='white')
